...
},createAndOpenNewSpace:function(d){TS.api.callSynchronously("files.createSpace",{},function(j,k,g){if(j){var i=TS.files.upsertAndSignal(k.file).file;
if(document.ssb_main&&document.ssb_main.TS){document.ssb_main.TS.files.upsertAndSignal(i)
}var e=(TS.model.active_cid?"?origin_channel="+TS.model.active_cid:"");
var f=i.permalink+e;
var h=function(){if(TS.client){return TS.client.windows.openFileWindow(i.id,e)
}else{if(document.ssb_main){if(document.ssb_main.TS){return document.ssb_main.TS.client.windows.openFileWindow(i.id,f)
}else{return true
}}}return false
};
if(!h()){TS.utility.openInNewTab(f,i.id)
}}else{TS.generic_dialog.alert('<p class="no_bottom_margin">Oops! Something went wrong. Please try again.</p>')
}if(typeof d=="function"){d(j,k,g)
}})
...
},getFileActions:function(e){if(!e){return
}var f={};
var g=false;
if(e.user==TS.model.user.id){g=true
}if(e.is_public){f.share=true
}else{if(g){f.share=true
}}f.comment=true;
if(!e.public_url_shared&&e.mode!="external"&&!TS.model.user.is_restricted&&!TS.model.team.prefs.disallow_public_file_urls){if(e.is_public){f.create_public_link=true
}else{if(g){f.create_public_link=true
}}}if(e.public_url_shared&&!TS.model.user.is_restricted&&(TS.model.user.is_admin||g)&&!TS.model.team.prefs.disallow_public_file_urls){f.revoke_public_link=true
}if(e.mode=="hosted"||e.mode=="snippet"){f.download=true
}if((e.mimetype&&e.mimetype.indexOf("image/")===0)||e.mode=="external"||e.mode=="snippet"||e.mode=="email"){f.open_original=true
}if(TS.web){if(e.mode=="post"||e.mode=="snippet"||e.mode=="space"||e.mode=="email"){f.print=true
}}if(g){if(e.mode=="snippet"||e.mode=="post"||e.mode=="space"){f.edit=true
}if(e.mode=="hosted"||e.mode=="email"){f.edit_title=true
}f.delete_file=true
}if(TS.model.user.is_admin){f.delete_file=true
}if(e.mode=="external"){if(g||TS.model.user.is_admin){f.refresh=true
}}if(window.Dropbox&&Dropbox.isBrowserSupported()&&TS.model.prefs.dropbox_enabled){if(e.mode=="hosted"){f.save_to_dropbox=true
}}if(TS.boot_data.feature_pins&&TS.qs_args.enable_pins==="1"){var d=TS.shared.getActiveModelOb();
if(d){if(e.pinned_to&&e.pinned_to.indexOf(d.id)!==-1){f.unpin_file=true
}else{f.pin_file=true
}}}return f
...
}else{TS.warn("WTF getActiveModelOb found no ob active_channel_id:"+TS.model.active_channel_id+" active_im_id:"+TS.model.active_im_id+" active_group_id: "+TS.model.active_group_id)
...
}}}else{if(K.event_ts){TS.ms.storeLastEventTS(K.event_ts)
...
},onFailure:function(M){TS.info("TS.ms.onFailure reason_str:"+M);
...
TS.info("_connect_timeout_tim_ms:"+y);
clearTimeout(G);
G=setTimeout(C,y);
TS.ms.last_url=L;
...
clearTimeout(h);
clearTimeout(G);
if(L){TS.info("_onDisconnect event:");
TS.error(L);
...
TS.info("Hello from: "+TS.model.team.url+" since_last_pong_ms:"+L);
...
}if(TS.boot_data.feature_flexpane_rework){var c=["channel_page_empty_state","channel_page_member_tabs","channel_page_member_lists","channel_page_member_row","channel_page_empty_pinned_items","channel_page_toggle"];
...
var a=false;
if(C.edit_msg||C.delete_msg||C.pin_msg||C.unpin_msg){a=true
...
},formatMessageByType:function(c,m,f){var h="";
var g;
if(c.ignore_if_attachments_supported){return h
}m=(m===true);
f=(f===true);
var j,n,a;
if(c._jl_rollup_hash&&c.user in c._jl_rollup_hash.users){j=TS.channels.getChannelById(TS.model.active_channel_id);
a=TS.members.getMemberById(c.inviter);
var o=c._jl_rollup_hash.users[c.user];
var l=TS.templates.builders.buildJoinLeaveRollUpStr(o);
l+=(j?" #"+j.name:" the channel");
if(o.is_in&&a){l+=" by invitation from <@"+a.id+"|"+a.name+">"
}var p=[];
var d=[];
var i="along with";
var b;
for(var e in c._jl_rollup_hash.users){if(e==c.user){continue
}b=c._jl_rollup_hash.users[e];
if(b.is_in===o.is_in){if(b.is_in){if(b.inviter==c.user){i="and invited"
}if(b.inviter&&(b.inviter==o.inviter||b.inviter==c.user)){p.push("<@"+e+">")
}else{d.push("<@"+e+"> "+TS.templates.builders.buildJoinLeaveRollUpStr(b))
}}else{p.push("<@"+e+">")
}}else{d.push("<@"+e+"> "+TS.templates.builders.buildJoinLeaveRollUpStr(b))
}}if(p.length){l+=", "+i+" "+p.join(", ")
}if(d.length){l+=". Also, "+d.join(", ")+"."
}h=TS.format.formatWithOptions(l,null,{no_highlights:true,no_specials:true})
}else{if(c.subtype=="channel_join"){j=TS.channels.getChannelById(TS.model.active_channel_id);
a=TS.members.getMemberById(c.inviter);
if(a){g="joined"+(j?" #"+j.name:" the channel")+" from an invitation by <@"+a.id+"|"+a.name+">";
h=TS.format.formatWithOptions(g,c,{no_highlights:true,no_specials:true})
}else{h="joined"+(j?" #"+j.name:" the channel")
}}else{if(c.subtype=="channel_leave"){j=TS.channels.getChannelById(TS.model.active_channel_id);
h="left"+(j?" #"+j.name:" the channel")
}else{if(c.subtype=="channel_name"){j=TS.channels.getChannelById(TS.model.active_channel_id);
h='renamed the channel from "'+c.old_name+'" to "'+c.name+'"'
}else{if(c.subtype=="channel_topic"){if(!c.topic){h="cleared the channel topic"
}else{h='set the channel topic: <span class="topic">'+TS.format.formatWithOptions(c.topic,c,{no_highlights:true,no_specials:true})+"</span>"
}}else{if(c.subtype=="channel_purpose"){if(!c.purpose){h="cleared the channel purpose"
}else{h='set the channel purpose: <span class="purpose">'+TS.format.formatWithOptions(c.purpose,c,{no_highlights:true,no_specials:true})+"</span>"
}}else{if(c.subtype=="group_join"){n=TS.groups.getGroupById(TS.model.active_group_id);
a=TS.members.getMemberById(c.inviter);
if(a){g="from an invitation by <@"+a.id+"|"+a.name+">";
h="joined"+(n?" "+TS.model.group_prefix+n.name:" the group")+" "+TS.format.formatWithOptions(g,c,{no_highlights:true,no_specials:true})
}else{h="joined"+(n?" "+TS.model.group_prefix+n.name:" the group")
}}else{if(c.subtype=="group_leave"){n=TS.groups.getGroupById(TS.model.active_group_id);
h="left"+(n?" "+TS.model.group_prefix+n.name:" the group")
}else{if(c.subtype=="group_name"){j=TS.channels.getChannelById(TS.model.active_channel_id);
h='renamed the group from "'+c.old_name+'" to "'+c.name+'"'
}else{if(c.subtype=="group_topic"){if(!c.topic){h="cleared the group topic"
}else{h="set the group topic: "+TS.format.formatWithOptions(c.topic,c,{no_highlights:true,no_specials:true})
}}else{if(c.subtype=="group_purpose"){if(!c.purpose){h="cleared the group purpose"
}else{h="set the group purpose: "+TS.format.formatWithOptions(c.purpose,c,{no_highlights:true,no_specials:true})
}}else{if(c.subtype=="group_archive"){n=TS.groups.getGroupById(TS.model.active_group_id);
h="archived"+(n?" "+TS.model.group_prefix+n.name:" the group");
if(TS.client&&n&&n.is_archived){if(TS.model.archive_view_is_showing){h+='. The contents will still be available in search and browsable in the <a target="_blank" href="/archives/'+n.name+'">archives</a>.'
}else{h+='. The contents will still be available in search and browsable in the <a target="_blank" href="/archives/'+n.name+'">archives</a>. 						It can also be un-archived at any time. To close it now, <a onclick="TS.groups.closeGroup(\''+n.id+"')\">click here</a>."
}}}else{if(c.subtype=="group_unarchive"){n=TS.groups.getGroupById(TS.model.active_group_id);
h="un-archived"+(n?" "+TS.model.group_prefix+n.name:" the group")
}else{if(c.subtype=="channel_archive"){j=TS.channels.getChannelById(TS.model.active_channel_id);
h="archived"+(j?" #"+j.name:" the channel");
if(TS.client&&j&&j.is_archived){if(TS.model.archive_view_is_showing){h+='. The contents will still be available in search and browsable in the <a target="_blank" href="/archives/'+j.name+'">archives</a>.'
}else{h+='. The contents will still be available in search and browsable in the <a target="_blank" href="/archives/'+j.name+'">archives</a>. 						It can also be un-archived at any time. To close it now, <a onclick="TS.channels.closeArchivedChannel(\''+j.id+"')\">click here</a>."
}}}else{if(c.subtype=="channel_unarchive"){j=TS.channels.getChannelById(TS.model.active_channel_id);
h="un-archived"+(j?" #"+j.name:" the channel")
}else{if(c.subtype=="me_message"){h="<i>"+TS.format.formatWithOptions(c.text,c,{do_inline_imgs:m})+"</i>"
}else{if(c.subtype=="play_sound"){h='played "'+c.sound+'"'
}else{h=TS.format.formatWithOptions(c.text,c,{do_inline_imgs:m,enable_slack_action_links:f})
}}}}}}}}}}}}}}}}}if(!h&&h!==""){TS.warn("no html msg.subtype:"+c.subtype);
return""
...
if(TS.model.prefs.temp_archive_viewer||(e&&!!TS.utility.msgs.getMsg(b.ts,a.msgs))){k='<a class="msg_right_link msg_jump" data-cid="'+a.id+'">Jump</a>'
...
if(TS.model.prefs.temp_archive_viewer||(c&&!!TS.utility.msgs.getMsg(b.ts,a.msgs))){i='<a class="star_jump msg_right_link" data-cid="'+a.id+'">Jump</a>'
...
var a=!!TS.client;
var h="";
h='<div data-real-src="'+TS.utility.htmlEntities(l.src)+'" class="clear_both msg_inline_img_holder msg_inline_holder '+(c?"":"hidden")+'" style="width:'+l.display_w+"px; height:"+(l.display_h+2)+'px; max-width: 100%;">';
var i="ctrl";
...
var g={file:c,show_file_actions_cog:false,is_message:true,show_preview:c.has_more};
...
},getMsgActions:function(d){if(!d){return
}var c={edit_msg:true,delete_msg:true};
var a=false;
if(d.user==TS.model.user.id){a=true
}if(!a){c.edit_msg=false
}else{if(TS.model.team.prefs.msg_edit_window_mins>-1&&(TS.utility.date.getTimeStamp()-TS.utility.date.toDateObject(d.ts))/60000>TS.model.team.prefs.msg_edit_window_mins){c.edit_msg=false
}else{if(TS.utility.msgs.automated_subtypes.indexOf(d.subtype)!=-1){c.edit_msg=false
}else{if(d.subtype=="file_upload"||d.subtype=="file_share"||d.subtype=="file_mention"||d.subtype=="file_comment"){c.edit_msg=false
}}}}if(!TS.model.team.prefs.allow_message_deletion){if(!TS.model.user.is_admin){c.delete_msg=false
}else{if(TS.model.active_im_id&&!a){c.delete_msg=false
}}}else{if(TS.model.active_im_id){if(!a&&d.user!="USLACKBOT"&&d.subtype!="bot_message"){c.delete_msg=false
}}else{if(!a){if(!TS.model.user.is_admin){c.delete_msg=false
}}else{if(TS.utility.msgs.automated_subtypes.indexOf(d.subtype)!=-1){if(!TS.model.user.is_admin){c.delete_msg=false
}}}}}if(d.is_ephemeral){c.delete_msg=true
}if(TS.boot_data.feature_pins&&TS.qs_args.enable_pins==="1"){var b=TS.shared.getActiveModelOb();
if(b){if(d.pinned_to&&d.pinned_to.indexOf(b.id)!==-1){c.unpin_msg=true
}else{c.pin_msg=true
}}}return c
...
},emojiGraphicReplace:function(l,n,m,h){var k=emoji.text_mode;
...
return j
},emojiReplace:function(g,f){if(!g){return""
}if(f){return emoji.replace_colons(g,function(h){return TS.format.tokenizeStr(f,h)
})
}return emoji.replace_colons(g)
...
if(TS.model&&TS.model.team&&TS.model.team.domain=="tinyspeck"){if(g.length){TS.warn("#"+f.attr("id")+" had "+g.length+" LINKS WITH EXT HREFS BUT NOT data-referer-safe! to fix it took "+(TS.utility.date.getTimeStamp()-h)+"ms");
TS.dir(0,g)
...
if(TS.model&&TS.model.team&&TS.model.team.domain=="tinyspeck"){if(g.length){TS.warn("#"+f.attr("id")+" had "+g.length+' LINKS WITH EXT HREFS BUT WITHOUT rel="noreferrer"! to add rel it took '+(TS.utility.date.getTimeStamp()-h)+"ms");
TS.dir(0,g)
}else{}}}},makeSureAllLinksHaveTargets:function(f){var h=TS.utility.date.getTimeStamp();
var g=[];
...
if(TS.model&&TS.model.team&&TS.model.team.domain=="tinyspeck"){if(g.length){TS.warn("#"+f.attr("id")+" had "+g.length+" LINKS WITH HREFS BUT WITHOUT TARGETS! to add targets it took "+(TS.utility.date.getTimeStamp()-h)+"ms");
TS.dir(0,g)
}else{}}TS.utility.makeSureAllExternalLinksAreRefererSafe(f)
...
}h=TS.utility.normalizeUrlForComparison(String(h));
...
},normalizeUrlForComparison:function(f){return f.replace(/\.dev[0-9]*.slack.com/i,".slack.com")
}});
...
var f=[];
var o=function(I,v,y,H){var L;
var G;
var E;
var N;
if(v.substr(0,1)=="#"){var D=v.substr(1);
E=D.split("|");
G=E[0];
L=TS.channels.getChannelById(G);
if(!L){L=TS.channels.getChannelByName(G)
}if(L){if(r||k){return TS.format.tokenizeStr(e,"#"+L.name)
}N=TS.utility.shouldLinksHaveTargets()?'target="/archives/'+L.name+'"':"";
if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(e,TS.format.generic_link_open+"#"+L.name+TS.format.link_close)
}return TS.format.tokenizeStr(e,'<a href="/archives/'+L.name+'" '+N+' data-channel-name="'+L.name+'" data-channel-id="'+L.id+'" class="internal_channel_link">#'+L.name+TS.format.link_close)
}else{if(E.length>1&&E[1]){return TS.format.tokenizeStr(e,"#"+E[1])
...
}}}}if(v.substr(0,1)=="@"){I=TS.utility.msgs.getMemberFromMemberMarkup(v);
if(I){if(r||k){return TS.format.tokenizeStr(e,"@"+I.name)
}N=TS.utility.shouldLinksHaveTargets()?'target="/team/'+I.name+'" ':"";
var O=(d)?"@"+I.name:TS.format.doHighlighting("@"+I.name);
if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(e,TS.format.generic_link_open+O+TS.format.link_close)
}return TS.format.tokenizeStr(e,'<a href="/team/'+I.name+'" '+N+'data-member-name="'+I.name+'" class="internal_member_link">'+O+TS.format.link_close)
}else{return TS.format.tokenizeStr(e,v)
}}if(v.substr(0,1)=="!"){var J=v.substr(1);
if(J){J=J.split("|")[0]
}if(["everyone","channel","group"].indexOf(J)!=-1){if(r||k){return TS.format.tokenizeStr(e,"@"+J)
}return TS.format.tokenizeStr(e,'<b class="mention">@'+J+"</b>")
}return TS.format.tokenizeStr(e,v)
}var M=v.split("|");
var w=M.shift();
w=w.replace(/\"/g,"&quot;");
var A=M.join("|")||w;
A=$.trim(A);
if(w.indexOf("<")===0){return TS.format.tokenizeStr(e,"&lt;"+v.replace(/</g,"&lt;").replace(/>/g,"&gt;")+"&gt;")
}A=A.replace(/</g,"&lt;");
A=A.replace(/>/g,"&gt;");
if(r){if(A==w){var B=(w.indexOf("//")>-1)?w.split("//")[1]:w;
B=jQuery.trim(B).substring(0,30).trim(this)+"...";
return""+B+""
}return"<"+A+">"
}else{if(k){return A
}}if(!t&&A!=w){A=TS.format.doSpecials(A,i&&i._special_debug)
}if(!n&&A!=w){A=TS.utility.emojiReplace(A,f)
}if(!d){A=TS.format.doHighlighting(A)
}var z;
if(w.indexOf(TS.utility.msgs.api_url_prefix+"chat.help")===0){if(q){z=TS.utility.htmlEntities(TS.utility.jsString(w));
return TS.format.tokenizeStr(e,'<a onclick="TS.utility.msgs.doApiUrl('+z+')" class="api_url">'+A+TS.format.link_close)
}else{return TS.format.tokenizeStr(e,'<a class="api_url muted">(Disabled) '+A+TS.format.link_close)
}}else{if(w.indexOf(TS.utility.msgs.new_api_url_prefix)===0){if(q){z=TS.utility.htmlEntities(TS.utility.jsString(w));
return TS.format.tokenizeStr(e,'<a onclick="TS.utility.msgs.doNewApiUrl('+z+')" class="api_url">'+A+TS.format.link_close)
}else{return TS.format.tokenizeStr(e,'<a class="api_url muted">(Disabled) '+A+TS.format.link_close)
}}else{if(w.indexOf("javascript:")==0){return TS.format.tokenizeStr(e,'<a onclick="'+w.replace("javascript:","")+'">'+A+TS.format.link_close)
}else{if(TS.client&&TS.client.core_url&&TS.utility.normalizeUrlForComparison(w).indexOf(TS.client.core_url)===0){if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(e,TS.format.generic_link_open+A+TS.format.link_close)
}return TS.format.tokenizeStr(e,'<a target="_self" href="'+w+'">'+A+TS.format.link_close)
}else{var K="";
var C;
if(i&&i.ts&&g){C=TS.inline_attachments.getAttachmentByFromUrl(i.attachments,w);
if(C){if(TS.boot_data.feature_attachments_inline||TS.templates.builders.shouldDoSimpleAttachment(C,i)){K=TS.templates.builders.buildAttachmentHTML({attachment:C,url:w,msg:i,show_initial_caret:true}).replace(/\n/g,"").replace(/\t/g,"").replace(/  /g," ")
}}}if(K){K=K.replace(/\n/g,"").replace(/\t/g,"").replace(/ +/g," ")
}if(TS.format.testing_with_generic_tokens){return TS.format.tokenizeStr(e,TS.format.generic_link_open+A+TS.format.link_close+K)
}var u=TS.utility.getFileIDFromURL(w);
var x=(u)?' class="file_preview_link"':"";
var F=(u)?' data-file-id="'+u+'"':"";
return TS.format.tokenizeStr(e,"<a "+TS.utility.makeRefererSafeLink(w)+' target="_blank"'+x+F+">"+A+TS.format.link_close+K)
}}}}};
s=s.replace(/<(.*?)>/g,o);
s=s.replace(/\</g,"&lt;");
s=s.replace(/\>/g,"&gt;");
...
var q;
var I;
if(r=="GROWL"||r=="EDIT"){for(z=0;
z<u.length;
z++){F=u[z];
if(F.indexOf("<")===0){if(H[F]){TS.error('unexpected: mode == "GROWL" || "EDIT", and yet we got something in the formatting map? '+F)
}else{if(F.indexOf("<!")===0){e=F.replace(/<|>|!/g,"");
y+="@"+e
}else{if(F.indexOf("<@")===0){e=F.replace(/<|>/g,"");
x=TS.utility.msgs.getMemberFromMemberMarkup(e);
if(x){y+="@"+x.name
}else{if(r=="EDIT"){y+=e
}else{if(r=="GROWL"){y+=F
}}}}else{if(F.indexOf("<#")===0){e=F.replace(/<|>|#/g,"");
q=e.split("|");
v=q[0];
G=TS.channels.getChannelById(v);
if(G){y+="#"+G.name
}else{if(q.length>1&&q[1]){y+="#"+q[1]
}else{if(TS.model.user.is_restricted){y+="#unknown-channel"
}else{y+="#deleted-channel"
...
}}}}}}}}else{if(F.indexOf("<")==-1){y+=TS.utility.unHtmlEntities(F)
...
z<u.length;
z++){F=u[z];
if(F.indexOf("<")===0){if(H[F]){y+=H[F];
if(F==TSF.LINK_END){y+=E;
E=""
}}else{if(F.indexOf("<!")===0){e=F.replace(/<|>|!/g,"");
y+='<b class="mention">@'+e+"</b>"
}else{if(F.indexOf("<@")===0){e=F.replace(/<|>/g,"");
x=TS.utility.msgs.getMemberFromMemberMarkup(e);
if(x){I=TS.utility.shouldLinksHaveTargets()?'target="/team/'+x.name+'" ':"";
var K=(g)?"@"+x.name:TS.format.doHighlighting("@"+x.name);
y+='<a href="/team/'+x.name+'" '+I+'data-member-name="'+x.name+'" class="internal_member_link">'+K+TS.format.link_close
}else{y+=e
}}else{if(F.indexOf("<#")===0){e=F.replace(/<|>|#/g,"");
q=e.split("|");
v=q[0];
G=TS.channels.getChannelById(v);
if(G){I=(TS.utility.shouldLinksHaveTargets())?'target="/archives/'+G.name+'"':"";
if(TS.format.testing_with_generic_tokens){y+=TS.format.generic_link_open+"#"+G.name+TS.format.link_close
}y+='<a href="/archives/'+G.name+'" '+I+' data-channel-name="'+G.name+'" data-channel-id="'+G.id+'" class="internal_channel_link">#'+G.name+TS.format.link_close
}else{if(q.length>1&&q[1]){y+="#"+q[1]
}else{if(TS.model.user.is_restricted){y+="#unknown-channel"
}else{y+="#deleted-channel"
}}}}else{if(F.indexOf(TSF.LINK_START.split(" ")[0])===0){var h=(function(i){var c=$.trim(i.replace(TSF.LINK_START.split(" ")[0],""));
return c.substr(0,c.length-1)
})(F);
if(h.indexOf(TS.utility.msgs.api_url_prefix+"chat.help")===0){if(D){n=TS.utility.htmlEntities(TS.utility.jsString(h));
y+='<a onclick="TS.utility.msgs.doApiUrl('+n+')" class="api_url">'
}else{y+='<a class="api_url muted">(Disabled) '
}}else{if(h.indexOf(TS.utility.msgs.new_api_url_prefix)===0){if(D){n=TS.utility.htmlEntities(TS.utility.jsString(h));
y+='<a onclick="TS.utility.msgs.doNewApiUrl('+n+')" class="api_url">'
}else{y+='<a class="api_url muted">(Disabled) '
}}else{if(h.indexOf("javascript:")===0){y+='<a onclick="'+h.replace("javascript:","")+'">'
}else{if(TS.client&&TS.client.core_url&&TS.utility.normalizeUrlForComparison(h).indexOf(TS.client.core_url)===0){y+='<a target="_self" href="'+h+'">'
}else{var f=TS.utility.getFileIDFromURL(h);
var k=(f)?' class="file_preview_link"':"";
var t=(f)?' data-file-id="'+f+'"':"";
y+="<a "+TS.utility.makeRefererSafeLink(h)+' target="_blank"'+k+t+">";
if(E){TS.error("WTF we should have no attach_html")
}E="";
var p;
if(l&&l.ts&&j){p=TS.inline_attachments.getAttachmentByFromUrl(l.attachments,h);
if(p){if(TS.boot_data.feature_attachments_inline||TS.templates.builders.shouldDoSimpleAttachment(p,l)){E=TS.templates.builders.buildAttachmentHTML({attachment:p,url:h,msg:l,show_initial_caret:true})
}}}}}}}}else{if(F.indexOf(TSF.EMOJI_COLONS.split(" ")[0])===0){var d=F.split(" ")[1].replace(">","");
if(!w){var C=TS.utility.emojiReplace(d);
if(C==d){if(g){y+=d
}else{y+=TS.format.doHighlighting(d)
}}else{y+=TS.utility.emojiReplace(d)
}}else{y+=d
}}else{if(F.indexOf(TSF.HEX_BLOCK.split(" ")[0])===0){var o=F.split(" ")[1].replace(">","");
if(!B){y+=' <div class="inline_color_block" style="background:'+o+';"></div>'
}}else{TS.error("markup token not handled:"+F)
}}}}}}}}else{if(F.indexOf("<")==-1){if(g){y+=F
}else{y+=TS.format.doHighlighting(F)
}}else{TS.error("token has a < in it but it is not the first character!\n"+F)
}}}}return y
}
})();
...
TS.emoji_menu.$emoji_menu_footer.html('<span id="emoticon_preview">'+TS.utility.emojiGraphicReplace(d)+'</span><div class=" overflow_ellipsis float_left"><span id="emoji_title">Emoji Deluxe</span><br /><span id="emoticon_name">'+d+"</span></div>");
...
}else{if(g=="member_skype_item"){}else{if(g=="member_account_item"){}else{if(g=="member_prefs_item"){f.preventDefault();
...
}else{if(g=="member_slackbot_responses"){}else{f.preventDefault();
...
TS.kb_nav.setSubmitItemHandler(TS.menu.onMemberItemClick);
var h=0;
if(!d.is_self){h=m.outerWidth()-TS.menu.$menu.width()-1
...
if(TS.boot_data.feature_flexpane_rework){TS.menu.positionAt($("#flex_menu_toggle"),-(TS.menu.$menu.width()-$("#flex_menu_toggle").width()),40);
...
}if(i.error=="not_starred"){if(TS.client&&TS.model.team.domain=="tinyspeck"){alert("tell eric not_starred (this message is for team tinyspeck only)")
}b(false)
}else{b()
}})
...
if(TS.client&&!TS.boot_data.feature_comment_mentions_autocomplete){i.TS_tabComplete({complete_cmds:true,complete_channels:true,complete_emoji:true,complete_member_specials:true,onComplete:function(m){TS.utility.populateInput(i,m)
},new_cmds:TS.boot_data.feature_cmd_autocomplete})
}if(TS.boot_data.feature_comment_mentions_autocomplete){i.TS_tabComplete2({complete_cmds:false,complete_channels:true,complete_emoji:true,complete_member_specials:true,no_tab_out:true,onComplete:function(m){TS.utility.populateInput(i,m)
...
return
}else{if(TS.boot_data.feature_comment_mentions_autocomplete&&i.tab_complete_ui("isShowing")){n.preventDefault();
return
}else{if(!n.shiftKey&&!n.altKey){n.preventDefault();
TS.msg_edit.checkAndSubmit(i,d);
return
}}}}}}if(TS.client){if(!TS.boot_data.feature_comment_mentions_autocomplete){if(n.which==TS.utility.keymap.tab){i.TS_tabComplete("onTabKey",n)
}else{i.TS_tabComplete("resetMatches","not tab key:"+n.which)
}}}}).autosize();
